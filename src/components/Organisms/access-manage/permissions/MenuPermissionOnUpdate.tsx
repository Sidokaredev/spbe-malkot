...
export default function MenuPermissionOnUpdate({
  open,
  anchorEl,
  setAnchorEl,
  selectedToEdit,
  ...
  >;
  selectedToEdit: { id: number; nama: string; deskripsi: string; aksi: string };
  ...
  const formOnSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const validating = UpdatePermission.safeParse(formValue);
    ...
    setLoading(false);
    setApiStatus("Berhasil memperbarui Hak Akses");
